...
```{r scatter_3-show, echo=FALSE, fig.width = 4, fig.height = 3}
ggplot(sparrows) + 
...
+ Your plot should look like this:
  ...
  ```{r scatter_4-show, echo=FALSE, fig.width = 4, fig.height = 3}
  ggplot(sparrows) + 
...
```{r dist_7-show, echo=FALSE, fig.width = 4, fig.height = 3}
ggplot(sparrows) +
...
+ Your plot should look like this (but maybe with colors/fills of your choosing??) - 
  ...
  ```{r dist_8-show, echo=FALSE, fig.width = 4, fig.height = 3}
  ggplot(sparrows) +
...
+ Instead of "just" a fill, modify the previous boxplot to fill distributions based on `Sex`. To accomplish this, you'll need to remove the "just a fill" and include a fill mapping in `aes()`. It should look like this:
  ...
  ```{r dist_9-show, echo=FALSE, fig.width = 4, fig.height = 3}
  ggplot(sparrows) +
...
+ Your plot should roughly look like this (but remember, jittering is random so it won't be identical!)
  ```{r dist_11-show, echo=FALSE, fig.width = 4, fig.height = 3}
  ggplot(sparrows) +
    aes(x = Sex, y = Wingspread) +
...
+ Your plot should roughly look like this (but remember, jittering is random so it won't be identical!)
  ```{r dist_12-show, echo=FALSE, fig.width = 4, fig.height = 3}
ggplot(sparrows) +
...
+ Your plot should look like this:
  ...
  ```{r bar_3-show, echo=FALSE, fig.width = 4, fig.height = 3}
ggplot(sparrows) + 
...
+ Hint: Literally all you need to do is, instead of writing `geom_bar()`, write it like this with the argument: `geom_bar(position = position_dodge())`
...
+ Seriosuly, insights you gain from these plots are CRITICAL to your understanding of `ggplot2`. Don't take this question lightly! Spend some time, take some of your own notes, and ask me questions!

```{r scatter2_7-1, exercise=TRUE}
ggplot(sparrows) +
  aes(x = Wingspread, y = Femur_Length, color = Survival) +
  geom_point() + 
  geom_smooth(method = "lm")
...
+ *Color* all points based on `Survival`
  + With an included *linear trendline* for EACH `Survival` level (there should be two trendlines in the end)
